...
<!-- 
恭喜你找到隱藏的資料 (bushi
因為 store 的性質非常特殊，所以在搞懂 docs 裡的其他部分之前是不應該使用的。
#### store
將 ps 或 vs 的值轉儲回到 ini 變量中。
store 是一個非常資源複雜性的運算保留字，使用過於頻繁可能會導致顯卡過載或遊戲閃退。
目前實際運用的地方有 HealthBar.ini 及 EnergyBar.ini (尚未釋出)，並且都有一個長達 30 幀的更新冷卻。
```ini
